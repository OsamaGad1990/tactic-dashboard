-- ============================================================
-- RPC: get_dashboard_ops_metrics v3.6 — Personnel Filter
--
-- Changes from v3:
--   1. NEW PARAM: p_staff_ids uuid[] DEFAULT NULL
--      - NULL = full team scope (no personnel filter)
--      - [id1, id2] = restrict to these specific users
--   2. scoped_visits: AND (p_staff_ids IS NULL OR vc.user_id = ANY(p_staff_ids))
--   3. scoped_notifications: AND (p_staff_ids IS NULL OR n.for_user = ANY(p_staff_ids))
--   4. Product availability: unchanged (market-scoped, not user-scoped)
--
-- This fixes the data leak where multiple users share the same
-- market, and selecting a Team Leader / User would still show
-- all users' data from those markets.
-- ============================================================

-- Drop ALL old signatures to avoid overload confusion
...
CREATE OR REPLACE FUNCTION public.get_dashboard_ops_metrics(
    ...
    p_staff_ids   uuid[] DEFAULT NULL   -- v3.6: Personnel filter
)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
...
BEGIN
    ...
    -- ═══════════════════════════════════════════════════════════
    -- SECTION 1: VISITS
    -- v3.6: Added p_staff_ids filter on user_id
    -- ═══════════════════════════════════════════════════════════
    scoped_visits AS (
        SELECT
            vc.id,
    ...
    -- v3.6: Inherits p_staff_ids from scoped_visits
    -- ═══════════════════════════════════════════════════════════
    ...
    -- ═══════════════════════════════════════════════════════════
    -- SECTION 3: NOTIFICATIONS — Scoped to Army + Staff Filter
    -- v3.6: Added p_staff_ids filter on for_user
    -- ═══════════════════════════════════════════════════════════
    ...
    -- ═══════════════════════════════════════════════════════════
    -- TOP PERFORMERS (inherits scoped_visits → p_staff_ids)
    -- ═══════════════════════════════════════════════════════════
    user_stats AS (
        SELECT
            sv.user_id,
...
-- Grant access to authenticated users (new 4-param signature)
