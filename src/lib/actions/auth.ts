...
/**
 * Sign in with activation code (OTP-style login)
 * For first-time users who received an activation code
 */
export async function signInWithCode(
    ...
    // Verify activation code via RPC
    ...
    if (verifyError || !verifyResult?.[0]?.valid) {
        return { error: 'invalid_code' };
    }

    const result = verifyResult[0];

    // Sign in the user using admin auth (since code was verified)
    // We need to use a special sign-in method or generate session
    const { error: signInError } = await supabase.auth.signInWithPassword({
        email,
        password: code, // Try with the code as password first
    });

    // If password sign-in fails, we need to handle it differently
    // For now, return success with redirect to change password
    if (signInError) {
        // The user needs to set their password via magic link or admin reset
        return {
            error: 'code_verified_password_required',
            success: false,
        };
    }
    ...
    // Return with password change requirement
    return {
        success: true,
        requirePasswordChange: result.account_status === 'pending' || result.account_status === 'must_change_password',
        portalRole: result.portal_role as PortalRole ?? 'none',
    };
}
