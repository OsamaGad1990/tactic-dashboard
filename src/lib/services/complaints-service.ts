...
import { complaints, complaintTargets, complaintTimeline } from '@/lib/db/schema';
...
async function getDivisionNames(ids: string[]): Promise<Map<string, { name: string | null; nameAr: string | null }>> {
    ...
    const idsLiteral = `{${ids.join(',')}}`;
    const result = await db.execute(sql`
        SELECT id, name, name_ar FROM client_divisions WHERE id = ANY(${idsLiteral}::uuid[])
    `) as unknown as { id: string; name: string | null; name_ar: string | null }[];

    for (const row of result) {
        map.set(row.id, { name: row.name, nameAr: row.name_ar });
    }
    return map;
}
...
async function getCategoryNames(ids: string[]): Promise<Map<string, { name: string | null; nameAr: string | null }>> {
    ...
    const idsLiteral = `{${ids.join(',')}}`;
    const result = await db.execute(sql`
        SELECT id, name, name_ar FROM categories WHERE id = ANY(${idsLiteral}::uuid[])
    `) as unknown as { id: string; name: string | null; name_ar: string | null }[];
...
export const getComplaints = cache(async (clientId: string, divisionId?: string | null): Promise<ComplaintRow[]> => {
    try {
        ...
        const conditions: SQL[] = [eq(complaints.clientId, clientId)];
        if (divisionId) {
        ...
        // Fetch targets & timeline using raw SQL ANY()
        const complaintIdsLiteral = `{${complaintIds.join(',')}}`;
        const [accountMap, marketMap, divisionMap, categoryMap, targets, timeline] = await Promise.all([
            ...
            db.execute(sql`
                SELECT complaint_id FROM complaint_targets WHERE complaint_id = ANY(${complaintIdsLiteral}::uuid[])
            `) as unknown as Promise<{ complaint_id: string }[]>,
            db.execute(sql`
                SELECT complaint_id FROM complaint_timeline WHERE complaint_id = ANY(${complaintIdsLiteral}::uuid[])
            `) as unknown as Promise<{ complaint_id: string }[]>,
        ]);

        // Count targets/timeline per complaint
