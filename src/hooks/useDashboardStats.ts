...
export function useDashboardStats(params: HookParams) {
  ...
  const run = useCallback(async () => {
    ...
    const callAV = async (store: string | null) => {
      ...
      const { data, error } = await rpcAvailabilityTotals({
      ...
      const r = (data?.[0] ?? null) as AvTotalsRow | null;
    ...
    // 3. استدعاء الدالة الجديدة المرنة مرة واحدة (بدون Generics/returns لتجنّب parsing issue)
    const { data: vcRaw, error: vcError } = await supabase.rpc(
      "get_dashboard_metrics",
      {
        p_client: clientId,
        p_region: eff.region,
        p_city: eff.city,
        p_stores: storesParam,
        ...
        // p_team_leader_id: eff.teamLeaderId, // لو أضفتها في الRPC
      }
    );

    // 👇 نعمل typing بعد النداء ونكمّل بنفس المنطق
    const vcData = (vcRaw ?? []) as VisitCardsRow[];
    const rr = !vcError ? vcData[0] ?? null : null;

    const total_visits      = Number(rr?.total_visits ?? 0);
    const finished_visits   = Number(rr?.finished_visits ?? 0);
    const unfinished_visits = Number(rr?.incomplete_visits ?? 0);
